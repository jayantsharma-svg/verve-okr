        ...
        <div className="bg-white rounded-2xl shadow-cap-md overflow-hidden">
          ...
          <div className="p-8">
            ...
            <div className="text-center mb-7">
              {/* Verve mark */}
              <div className="inline-flex items-center justify-center w-12 h-12 bg-cap-blue rounded-xl mb-4 shadow-cap-sm">
                <span className="text-white font-extrabold text-xl italic tracking-tight">V</span>
              </div>
              <h1 className="text-2xl font-extrabold text-ink-900 tracking-tight">Sign in to Verve</h1>
              <p className="text-sm text-ink-500 mt-1 font-medium">Capillary Technologies</p>
            </div>
              ...
              {(['sso', 'password'] as const).map((m) => (
                <button
                  ...
                  className={cn(
                    ...
                    mode === m
                      ? 'bg-white text-cap-blue shadow-cap-sm'
            ...
            {mode === 'sso' ? (
              <button
                ...
                className="w-full flex items-center justify-center gap-3 border-2 border-ink-200 rounded-xl px-4 py-3 text-sm font-semibold text-ink-700 hover:border-cap-blue hover:text-cap-blue transition-colors group"
              >
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    autoComplete="email"
                    {...register('email')}
                    className="w-full border border-ink-200 rounded-xl px-4 py-2.5 text-sm text-ink-900 placeholder:text-ink-400 focus:outline-none focus:ring-2 focus:ring-cap-blue focus:border-transparent transition-shadow"
                ...
                <div>
                  ...
                  <input
                    ...
                    {...register('password')}
                    className="w-full border border-ink-200 rounded-xl px-4 py-2.5 text-sm text-ink-900 focus:outline-none focus:ring-2 focus:ring-cap-blue focus:border-transparent"
                  />
                  {errors.password && <p className="text-cap-red text-xs mt-1">{errors.password.message}</p>}
                </div>
                <div className="flex justify-end">
                  <a href="/login/forgot-password" className="text-xs text-cap-blue hover:underline font-medium">Forgot password?</a>
                </div>
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="w-full bg-cap-blue text-white rounded-xl py-3 text-sm font-bold hover:bg-cap-blue-d disabled:opacity-50 transition-colors shadow-cap-sm"
                >
